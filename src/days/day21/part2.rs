...
pub fn part2(path: &str) -> u64 {
    ...
    for line in input.lines() {
        ...
        sum += num * get_shortest(&line, &mut cache);
    }
    sum
}

fn get_shortest(code: &str, cache: &mut HashMap<(char, char, u8), u64>) -> u64 {
    let mut result = 0;
    let code = "A".to_string() + code;
    for i in 0..code.len() - 1 {
        let mut code_it = code.chars();
        let s = &code_it.nth(i).unwrap();
        let t = &code_it.next().unwrap();

        let mut seen: HashSet<char> = HashSet::new();
        let mut q: Vec<(char, String)> = Vec::from([(*s, "".to_string())]);
