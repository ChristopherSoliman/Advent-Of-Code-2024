...
pub fn part1(path: &str) -> u32 {
    ...
    for line in input.lines() {
        let num = line.strip_suffix("A").unwrap().parse::<u32>().unwrap();
        sum += num * get_code_length(&line);
    }
    sum
}
...
fn get_code_length(code: &str) -> u32 {
    let mut result = 0;
    let code = "A".to_string() + code;
    for i in 0..code.len() - 1 {
        let mut code_it = code.chars();
        result += get_shortest(&code_it.nth(i).unwrap(), &code_it.next().unwrap());
    }
    result
}

fn get_shortest(num1: &char, num2: &char) -> u32 {
    ...
    paths
        ...
        .unwrap()
        .iter()
        .map(|v| get_lowest_score(v.to_owned() + "A", 2))
        .min()
        .unwrap()
}

fn get_lowest_score(code: String, level: u8) -> u32 {
    let mut result = 0;
    let code = "A".to_string() + &code;
